...
export default function Chat() {
  ...
  */
  /*
    ...
    записывает человек.
  */
  const [pending, setPending] = useState<{ text: string; label: string }[]>([]);
  ...
  */
   ...
   */
  const confirmPending = async (item: { text: string; label: string }) => {
  ...
  const skipPending = (item: { text: string; label: string }) =>
    setPending((p) => p.filter((x) => x.text !== item.text));
      ...
      try {
        ...
        if (added.length) {
          ...
          for (const a of added) {
            ...
            // РАЗВИЛКА: своё пишем, придуманное спрашиваем. Разбор — у `pending` выше.
          ...
          if (ask.length) setPending((p) => [...p, ...ask.filter((x) => !p.some((y) => y.text === x.text))]);
          setJustAdded((p) => {
...
function StepWidget({
  ...
  onConfirmPending?: (item: { text: string; label: string }) => void;
  onSkipPending?: (item: { text: string; label: string }) => void;
...
function HobbyW({ mapOpen, mapPicked, onMapPick, onMapDone,
  ...
  */
  if (pending && pending.length) {
    ...
    return (
      <View style={cs.widget}>
        <Hint>{CONFIRM_INTEREST.ask()}</Hint>
        <View style={cs.row}>
