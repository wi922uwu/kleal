...
console.log('\nправка интента со сводки происходит в самом листе');
{
  ...
  check('правка применяется по выбранному target и по кнопке',
    /applyIntentEdit\(current, edraft, editPick\)/.test(sheet),
    'без белого списка точечный редактор перезапишет соседние параметры');
  ...
  check('черновик пересевается при каждом открытии',
    /const openEdit = \([\s\S]{0,260}setEdraft\(beginIntentEdit\(draft\)\)/.test(wiz)
    && /onEdit=\{\(\) => openEdit\(\)\}/.test(wiz),
  ...
  // Тема остаётся исходной фразой без отдельного шага, а реальные параметры доступны напрямую.
  check('режим и формат правятся, а не отправляют в мастер',
    !/EDIT_SHEET\.theme/.test(wiz) && /EDIT_SHEET\.mode\(\)/.test(wiz)
    && /EDIT_SHEET\.format\(\)/.test(wiz) && /onEditField=\{openEdit\}/.test(wiz),
    'summary обязан вести прямо в контрол режима/формата');
  ...
  check('пояс разворачивается внутри листа, а не вторым модалом',
    !/setTzOpen\(true\)/.test(sheet) && /k === 'timezone'[\s\S]{0,180}tzOptions\(\)\.map/.test(wiz),
  ...
  check('раскрыто ровно одно поле',
    /editPick === k \? setEditPick\(''\) : chooseEditTarget\(k\)/.test(sheet)
    && /setEdraft\(beginIntentEdit\(draft\)\)[\s\S]{0,100}setEditPick\(target\)/.test(wiz),
  ...
  const dead = keys.filter((k) => !field.includes(`'${k}'`));
...
console.log('\nGroup Online проходит весь подтверждённый canvas-флоу');
{
  ...
  check('пустая или битая ссылка не проходит ни Next, ни правку сводки',
    ...
    && /editBlocked\(edraft, editPick\)/.test(wiz)
