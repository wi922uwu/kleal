...
# ОДИН СТОРОЖ НА МАШИНУ, и это не гигиена, а лечение.
#
# Два сторожа не просто дублируют работу — они ломают ровно то, ради чего сторож существует.
# Каждый проверяет адрес из общего .expo-url, но поднимает СВОЙ туннель и вписывает туда своё имя.
# Второй читает чужое имя, идёт по нему в свой порт, не находит и объявляет «адрес не отвечает» —
# после чего меняет имя. Первый делает то же самое со своей стороны. Проверено вживую 11 августа:
# два сторожа сменили адрес трижды за пять минут, и каждый QR умирал раньше, чем его сканировали.
#
# Замок на дескрипторе, а не файл с PID: он снимается сам при любой смерти процесса, включая
# kill -9 и обрыв ssh, — а PID-файл в этих случаях остаётся и запирает машину навсегда.
LOCK="${LOCK:-/tmp/kleal-expo-tunnel.lock}"
exec 9>"$LOCK" || true
if command -v flock >/dev/null 2>&1 && ! flock -n 9; then
  echo "сторож уже работает (замок $LOCK) — второй не нужен, выхожу" >&2
  exit 0
fi
