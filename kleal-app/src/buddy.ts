...
export const CREATE = {
  ...
  /**
   * Тема собрана. Кнопка называет то, что произойдёт — «Создать интент», а не «Дальше»: круг
   * замыкается на том же действии, каким он начинался в окне Бадди, и человек не гадает, куда
   * его ведут. Уточнил детали ещё раз — снова сводка и снова эта кнопка.
   */
  /*
    НА КНОПКЕ НЕТ СЛОВА «ИНТЕНТ». Оно внутреннее: человеку приходится его переводить ровно в тот
    момент, когда от него ждут одного движения. Осталось само движение — «Нажми».
  */
  ready: () => T('Нажми', 'Tap'),
