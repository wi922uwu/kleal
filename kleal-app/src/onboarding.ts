 ...
 *
 * Поэтому заголовков два. «Поздравляем, ты в игре!» — это про только что законченный онбординг, и
 * говорится ОДИН раз. Тот, кто просто открыл приложение, поздравления с завершением не заслужил и
 * не поймёт: для него обычное приветствие. Настоящая главная (Agent Home) ещё не перенесена.
 */
export const DONE_SCREEN = {
  title: () => T('Поздравляем!\nТы в игре!', 'Congratulations!\nYou are on the board!'),
  home: (name: string) =>
    name ? T('Привет, ' + name + '!\nЧем займёмся?', 'Hi ' + name + '!\nWhat shall we do?')
         : T('Чем займёмся?', 'What shall we do?'),
