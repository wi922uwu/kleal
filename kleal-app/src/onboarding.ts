...
/** Опросник «помоги разобраться» — подписи под сценами и подборкой. */
export const SCENE = {
  /** `multi` — единственный вид с подтверждением: пока человек отмечает, отправлять нечего. */
  multiCta: (n: number) => T(`Ответить (${n})`, `Answer (${n})`),
  multiEmpty: () => T('Отметь, что узнаёшь', 'Mark what you recognise'),
  addCta: (n: number) => T(`Добавить (${n})`, `Add (${n})`),
  addEmpty: () => T('Отметь, что подходит', 'Mark what fits'),
  /** Выход из подборки, когда не подошло ничего. Один круг сцен, дальше — словами. */
  none: () => T('Ни одно не про меня', 'None of these are me'),
};
