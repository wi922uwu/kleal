...
# ЧУЖАЯ ПИСЬМЕННОСТЬ В ОТВЕТЕ. Модель роняет в русскую фразу иероглифы: «Интересно, какие方面ы
# культуры тебя больше всего» — так это увидел живой человек в анкете. Починить строкой нельзя:
# испорчено само слово, и вырезание иероглифов оставляет «какиеы». Значит, ответ негодный целиком.
#
# Сторож срабатывает ТОЛЬКО когда письменности не было в самом разговоре: человек, спросивший про
# 日本語, вправе получить это слово обратно.
_FOREIGN_SCRIPT = re.compile(
    r"[\u0590-\u05ff\u0600-\u06ff\u0700-\u074f\u0900-\u097f\u0e00-\u0e7f"
    r"\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uac00-\ud7af\uf900-\ufaff]")


def _off_script(reply, source):
    """Модель принесла письменность, которой в разговоре не было."""
    return bool(_FOREIGN_SCRIPT.search(str(reply or ""))) and not _FOREIGN_SCRIPT.search(str(source or ""))
...
# ОТКЛИК ПЕРЕД ТЕМ, КАК ВЕСТИ ДАЛЬШЕ. Дежурная фраза выдаётся, когда модель повторяется, — и в
# этом виде она глухая: человек написал «Я мама», а в ответ пришло голое «А кроме этого?». Так это
# и прочли — «отвечает херню». Отклик не притворяется, что что-то записано: он говорит «услышал».
_ACK = {
    "ru": ["Понял.", "Ясно.", "Хорошо."],
    "en": ["Got it.", "I see.", "Alright."],
    "es": ["Entendido.", "Ya veo.", "Vale."],
}
...
def interests_chat(messages, profile, lang="ru", recorded=None):
    ...
    # чинят подстановками. Холодная температура на повторе, чтобы не получить ту же лотерею.
    obj, reply = {}, ""
    for _temp in (0.3, 0.0):
        try:
            raw = llm_complete(MODEL_ID, [{"role": "system", "content": sys_p},
                                          {"role": "user", "content": convo[-4000:]}], _temp)
            obj = base._extract_json(str(raw or "")) or {}
        except Exception:
            obj = {}
            break
        reply = base.polish_reply(str(obj.get("reply") or ""))[:300]
        if not _off_script(reply, convo):
            break
        print("[interests] чужая письменность в ответе, температура=%s" % _temp, flush=True)
    # Не выправилась и со второго раза — реплику отдаём дежурной, а записанное оставляем: оно
    # проверено цитатой из реплики человека и иероглифами не испорчено.
    if _off_script(reply, convo):
        reply = ""
    ...
    if not reply or _BAD_QUESTION.search(reply) or _asked_before(reply, msgs, skip_words=_names):
        ...
        # Человек только что рассказал о себе — уводить дальше, не откликнувшись, нельзя.
        if last_user.strip() and not _EMPTY_ANSWER.match(last_user.strip()):
            _acks = _ACK.get(lang) or _ACK["en"]
            reply = _acks[_turn % len(_acks)] + " " + reply
