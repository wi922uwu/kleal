...
def buddy_chat(messages, profile, signals, uid=None, on_text=None):
    ...
    obj = None
    # СОГЛАСИЕ НА СВОЁ ЖЕ ПРЕДЛОЖЕНИЕ НЕ ПЕЧАТАЕТСЯ.
    #
    # Агент предложил найти кого-нибудь, человек ответил «да» — и на экране одновременно
    # печаталась реплика и выезжало окно выбора. Печатать тут вообще нечего: реплика в этом ходе
    # формальность, содержание хода — окно. Клиент её и не оставлял: он снимал напечатанное, как
    # только узнавал, что пришёл интент, — то есть текст появлялся ради того, чтобы тут же
    # пропасть. Человек видел, как приложение печатает и стирает.
    #
    # Знать это заранее МОЖНО: `_agreed` считается по истории, до всякой модели. Поэтому на таком
    # ходу поток просто не открывается — идут те же три точки, что и на любом ожидании, а дальше
    # окно. Если интент вдруг не сложится, ответ придёт целиком в `done`, и клиент напечатает его
    # одной репликой: цена ошибки — потерянная скорость на одном ходу, не более.
    _stream = on_text if not _agreed else None
    ...
    for _attempt in range(2):
        ...
        try:
            ...
            if _stream is not None and _attempt == 0:
                raw = llm_stream(MODEL_ID, _cmsgs, 0.35, "reply", _stream)
            else:
