...
def _v2_union_interests(prior, fresh, merged):
    ...
    for x in prior_items + lst(fresh) + lst(merged):
        k = x.strip().lower()
        # The extractor may discover a more specific phrase from conversation, but discovery is not
        # consent. Built-in taxonomy keys are already visible choices; a novel phrase must go
        # through interest-normalize and the confirmation popup instead of appearing silently.
...
def _canon_interests(words):
    """Give a confirmed canonical interest extra handles the ranker can resolve.
    ...
    The confirmed canonical wording stays FIRST; filtration's related canonical topics are
    appended. The unconfirmed raw sentence never reaches this function. Best-effort by design: a
...
def attach_profile(login, name, profile=None):
    ...
    with _ACC_LOCK:
        ...
        if isinstance(profile, dict):
            ...
            # Confirmation receipts are short-lived server capabilities, not profile facts. Keeping
            # them in the credential file would make an expired implementation detail look durable.
...
def update_user(name, patch):
    ...
    with _REG_LOCK:
        ...
        if isinstance(clean.get("interests"), list):
            ...
            # A regular profile patch may remove/toggle existing interests and may add keys from the
            # built-in catalogue. A novel free-text phrase can enter only through interest-confirm,
            # which puts the validated canonical value in the row before this full-list sync runs.
...
def confirm_interest(name, token, owner_key=""):
    """Explicit confirmation is the only writer for a novel free-text interest."""
    ...
    key = str(name or "").strip().lower()
    if not key:
        # During onboarding the profile row does not exist yet. The confirmed receipt travels in
        # device state and register_profile validates it before the first database write.
    ...
    with _REG_LOCK:
        ...
        next_interests = _canon_interests(current) or current
        if canonical not in next_interests:
            return {"ok": False, "error": "interest limit reached"}
        row["interests"] = next_interests
        _persist_users(users, touched=row)
    return {"ok": True, "canonical": canonical, "label": rec.get("label"), "token": token,
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if p == "/api/onboarding/state":
            ...
            # The model has no write path. Only this explicit second request marks a proposal as
            # confirmed and, for an existing profile, appends its canonical key atomically.
            owner = session_owner(_bearer(self))
            if not owner:
                return send_json(self, 200, {"ok": False, "error": "no session"})
            ...
            send_json(self, 200, confirm_interest(requested_name, body.get("token"), owner.get("_key") or ""))
