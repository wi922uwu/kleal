...
"""Массовый прогон разговора об интересах: метрики вместо впечатлений.
    ...
    python3 tools/interests_chat_bench.py отчёт.json

Девяносто сценариев по пять ходов — 450 реплик. Сценарий = вступительная фраза плюс короткие
ответы, как отвечает живой человек: место, частота, компания, а в третьем наборе — одни «все» и
«да». Ответы НАРОЧНО одинаковы для всех интересов: так видно, приклеивает ли агент чужое
обстоятельство («пишу код» + «с лодки» -> «Кодинг с лодки» — так и было).

Шесть измеряемых болезней, все найдены на живом экране, а не придуманы:
    повтор вопроса          пять переформулировок одного вопроса подряд
    два вопроса в реплике   «что нравится, ритм или что-то другое?»
    телесная надобность     «пить воду» записано интересом
    дубль чипа              три чипа про одну рыбалку
    длинная подпись         чип-цитата вместо имени занятия
    подпись глаголом        «Писать код» вместо «Программирование»

ПОТОКОВ ТРИ. На шести модель отваливалась по таймауту на каждом пятом сценарии, и метрики
считались по неполным данным — 0.83 чипа на сценарий читалось как регрессия, хотя это были
недосчитанные строки.

Быстрая версия того же — tools/interests_chat_smoke.py: по одному сценарию на болезнь.
"""
...
def run(scene, out):
    ...
    rec = {"tag": scene["tag"], "turns": 0, "repeat": 0, "bodily": 0, "dupe": 0,
           "long_label": 0, "verb_label": 0, "two_q": 0, "chips": [], "asked": []}
...
def main():
    ...
    for k, ru in (("repeat", "повтор вопроса"), ("two_q", "два вопроса в реплике"),
